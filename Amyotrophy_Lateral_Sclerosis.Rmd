...
Computing the MAD, and dividing by columns.
This is some stuf that is not needed for the this data, becouse we see from the boxplot that are good enough.

## Integrating the data set with more information
...
Now we take the log, even though is not necessary and perform a PCA analysis.
Is possible to color the point, but we have to know the division in, for example, ctrl and treated.
...
I'll have to make a choice of which gene analyse, so we will using the p-value to make a filter to all the genes.
The data has some noise, by chance maybe some p-values will be interesting even if it isn't.
I perform a t.test for every genes on the data and keep all the gene with a p-value significant at 0.95 level
...
Now we have a vector with a p-value for every gene, so we set a threshold to filter the results of
...
#als_data <- als_data[,colnames(als_data)%in%query$external_gene_name]
```
...
Outliers are important because can move the centroid so return but clastering.
So a good idea is to remove them.
...
table(groups)
plot(hc_res, hang = -1,labels = rownames(als_data))
...
summary(results) 
ggplot(results) + labs(title = "Accuracy with t_test")
```
...
results <- resamples(list(RF = fit.rf.2.var, LDA = fit.lda.2.var, Lasso = fit.lasso.var, SVM_L = svm.lin.fit.var, SVM_P = svm.pol.fit.var))
...
trainRes <- scudoTrain(scudo_train, groups = f[train],  nTop= 25, nBottom= 25, alpha = 0.05) 
trainRes 
...
classRes <- scudoClassify(scudo_train, scudo_test, N = 0.25, nTop = 25, nBottom = 25, trainGroups = f[train], alpha = 0.5)
caret::confusionMatrix(classRes$predicted, f[test])
mean(classRes$predicted == f[test])
```
...
buttarli su string, fare pathfinder, e pathviw che usa kegg.
Magari tenere gene profiler e vedere cosa cambia, oppure cercare i genei singoli.
